...
def process_images(input_path, output_dir, cfg, guidance_scale=1.0, steps=50, seed=42, crop_size=192, segment=True, rescale=False, write_image=False):
    ...
    return base_name  # Return the base name or any identifier to use for the scene name


def update_yaml_config(output_dir, crop_size, guidance_scale):
    yaml_path = './instant-nsr-pl/configs/neuralangelo-ortho-wmask.yaml'
    ...
    recon_dir = os.path.join(output_dir, f"cropsize-{int(crop_size)}-cfg{guidance_scale:.1f}")
    scene_name = 'scene' + datetime.now().strftime('@%Y%m%d-%H%M%S')

    config['dataset']['root_dir'] = recon_dir
...
def run_reconstruction(yaml_path):
    subprocess.run(
        f'cd instant-nsr-pl && python launch.py --config {yaml_path} --gpu 0 --train && cd ..',
        shell=True,
    )
...
if __name__ == '__main__':
    ...
    scene_name = update_yaml_config(args.output_dir, args.crop_size, args.guidance_scale)
    run_reconstruction('./configs/neuralangelo-ortho-wmask.yaml')
